...
export default function MotelPageRenderer({
  ...
  return (
    <div className={`min-h-screen ${config.bg} ${config.text} ${config.selection}`} style={{ fontFamily: config.customFont }}>
      ...
      {/* Barra fija inferior estilo Airbnb/Booking: precio + contacto siempre a la mano */}
      {!isPreview && (
        <div className={`fixed bottom-0 inset-x-0 z-40 border-t border-white/10 bg-black/90 backdrop-blur-md`}>
          <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
            <div className="min-w-0">
              {showPrices && minPrice ? (
                <>
                  <p className={`text-lg font-bold leading-tight ${config.accent}`}>Desde ${minPrice}</p>
                  <p className="text-[9px] text-stone-500 uppercase tracking-widest">por sesión</p>
                </>
              ) : (
            ...
            <div className="flex items-center gap-2 shrink-0">
              {firstWhatsapp ? (
                <a
                  href={`https://wa.me/${firstWhatsapp}?text=${encodeURIComponent(`Hola, me interesa obtener información sobre ${place.name}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-green-500 hover:bg-green-600 text-white rounded-full text-[10px] font-black uppercase tracking-widest transition-all"
                >
                ...
                <a
                  href={`tel:${semantic_data.phone.split(/[,/|y]/)[0].trim().replace(/\D/g, '')}`}
                  className={`inline-flex items-center gap-1.5 px-5 py-2.5 ${config.accentBg} text-white rounded-full text-[10px] font-black uppercase tracking-widest transition-all`}
                >
                  <Icons.Phone />
                  Llamar
                </a>
