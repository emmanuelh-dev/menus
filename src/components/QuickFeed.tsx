...
export default function QuickFeed({ placeId }: QuickFeedProps) {
  ...
  const handleSubmit = async () => {
    if (!text && !image) {
      alert('Por favor agrega una foto o una instrucción.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/ai/update-content', {
        ...
        body: JSON.stringify({
          placeId,
          image: base64Image,
          instruction: text || (base64Image ? 'Analiza la imagen y actualiza el contenido.' : undefined)
        })
      });
      ...
      if (result.success) {
        alert('✓ ¡Gracias! Hemos actualizado la información con IA.');
        setIsOpen(false);
        setText('');
        setImage(null);
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#1A1A1A] border border-white/10 w-full max-w-md p-6 rounded-2xl shadow-2xl text-stone-300">
            ...
            <p className="text-sm text-stone-400 mb-6">
              Sube una foto del menú o describe lo que falta. Nuestra IA actualizará la página automáticamente.
            </p>

            <div className="space-y-4">
              <div 
                onClick={() => fileInputRef.current?.click()}
                className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer transition-colors ${image ? 'border-green-500 bg-green-500/5' : 'border-white/10 hover:border-white/20 hover:bg-white/5'}`}
              >
                <input 
                  type="file" 
                  ref={fileInputRef} 
                  ...
                  accept="image/*"
                />
                <svg xmlns="http://www.w3.org/2000/svg" className={`size-10 mb-2 ${image ? 'text-green-500' : 'text-stone-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span className="text-sm text-stone-500 text-center">
                  {image ? image.name : 'Haz clic para subir foto del menú o lugar'}
                </span>
              </div>
